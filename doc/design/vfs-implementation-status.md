...
- `SessionWorkbench` 使用 vfs-ui 与可写 SessionBrowser 投影（`createSessionBrowser` 以 `access: 'rw'` 创建，实现位于 `packages/app-core/src/session/session-browser.ts`）。Session 默认 main 聊天，直属仅 tasks/files；Task 查询执行历史，映射文件通过所属 Session 上下文打开和保存。投影支持 Session/文件夹 CRUD 与 `/files` 下的文件 CRUD；files 直接代理实际受限上下文，不依赖隐藏系统目录。分支草稿独立保存，旧手写侧栏已删除。接口与 C4 见 [Session 浏览实现](vfs-session-browser.md)。
...
- SessionFS 是文件能力边界，不是操作系统 sandbox；原生 IO 的路径检查也不是抵抗外部宿主恶意换链的完整 OS 沙箱。Tauri 受限 Session 不再注入无约束宿主 shell、原生 skill handler 或本地 Codex app-server；进程执行改由 Bubblewrap runner 实施（`apps/tauri-app/src-tauri/src/session_bash.rs`、`apps/tauri-app/src/shell/session-bash.ts`，经 `packages/app-core/src/vfs/session-process-context.ts` 与 `apps/tauri-app/src/main.ts` 接线）：除固定的运行库/系统资源外只绑定显式 Session 挂载，只读挂载用 `--ro-bind`、可写挂载用 `--bind`，cwd 必须落在授权挂载内，不继承宿主环境变量。它不隔离网络，也不是完整 OS 沙箱。
