...
TauriSkillSource 通过注入的 Session ToolVFSContext 读取 `_agent/AGENT.md` 和沿途 `_agent/skills/*/SKILL.md`，不使用 Tauri 原生文件 API。项目根是该作用域创建时的 Session 虚拟 cwd，构建 projectRoot → cwd 的目录链；项目根标 global-fs，中间层标 parent-fs，末级标 local-fs。cwd 在项目根外返回空定义，清理原项目作用域。**多层级规则取舍（2026-09-13 定稿）**：项目规则只取项目根的 `_agent/AGENT.md`，parent-fs/local-fs 层级的 `_agent/AGENT.md` 既不合并也不替换——切换 cwd 不能静默换掉项目指令；Skill 仍按层级级联发现，同名 id 由更深层级覆盖（`loadScope` 按 projectRoot→cwd 顺序返回，消费者按序写入时后者生效）。回归见 `session-file-source.test.ts`。不存在自动向上查找项目根的 findProjectRoot/buildScopeEntries 公共实现。Web/CLI 未配置 SkillSource 时不扫描宿主文件系统。
...
Glob matcher 支持 `*`、`**`、`?`、`{a,b}`。mountByGlob 记录 skillId 对应文件集合并标记 loaded；关闭最后一个匹配文件时卸载并清理所注册工具。**编辑器 open/close 接线已于 2026-09-11 完成**：`SessionSkillControls` 新增 `mountByGlob(sessionId, filePath)` / `unmountByGlob(sessionId, filePath)`（经 Session 操作队列串行化，只校验非空路径），`SessionWorkbench` 在打开文件编辑器/预览时挂载、在关闭或切换资源时卸载（挂载是**瞬时**的，不写入持久 loaded 身份，重新打开文件即恢复；失败只报告不打断编辑器）。回归：`packages/kernel-adapters/src/skill/session-skill-controls.test.ts`「mounts and unmounts editor targets through the Session scope in order」、`packages/app-shell/tests/session-browser-ui.test.ts`（打开 `/workspace/note.md` 触发挂载、切走后卸载）。

旧文档中的 ContextManager/P0–P4、固定 4000 token 门控和 applyL3LLMSummarize 已不在当前执行链。当前 ContextAssembler 接受 skillsPrompt；FlowNodeBinder 显式解析 capabilityPolicy/节点 skillIds，排除 disabled/disableModelInvocation 定义，将已选技能正文、Compact Instructions 和工具 ID 合并到节点配置。关键规则以独立 system 消息进入 Task input，沿用逐轮裁剪的 system 保护；systemPromptPolicy=none 仍不注入任何系统提示。app-core 装配的 resolveSessionContext（[create-application-runtime.ts](../../packages/app-core/src/runtime/create-application-runtime.ts)）经 [kernel-adapters/src/skill/session-prompt-context.ts](../../packages/kernel-adapters/src/skill/session-prompt-context.ts) 调用 buildSkillPromptContext：刷新来源后将 L2 序列化为仅 id/name/description 的索引，将 L3/L4 已加载定义的正文、支持文件和关键规则合入 session-skill 块；L1 不注入。ContextAssembler 持久保存 project/session-skill/skill-index 系统来源，直接聊天及聊天内 Flow 消费同一快照。该路径的「关键规则」只取 `compact.redLines`（`- [红线] …`）条目（`aggregateCompactInstructions`），非红线的 compact 正文不进入 session-skill 块；直接聊天的 `skillsPrompt` 使用 `compact.rawContent`（含非红线条目）。未加载正文不进入索引，构建失败不提交新 Task，工具白名单不因此扩大。L2 索引默认最多 8192 UTF-8 字节（包含提示头和 JSON），按 priority/id 排序保留完整元数据条目并报告 omitted 数量；过大条目跳过，仍可容纳后续较小条目。buildSkillPromptContext.indexByteLimit 可调整，0 禁用，非法值在读取/加载前拒绝。ContextAssembler 总预算不足时先丢弃 skill-index，再裁剪历史；规则仍受保护。字节上限不是模型 token 精确上限，逐轮 Agent 裁剪仍保留已进入 Task 的 system 消息；新运行将当前用户消息传入匹配器，按 priority/id 顺序加载启用且非静默、非 action 的候选：autoLoad=true 或启发式匹配命中。成功后用 Session shared CAS 合并 loaded ID，持久化失败卸载本次候选并终止提交；此前成功保存的候选仍保留。自动与显式 Effect 加载共用 rememberLoadedSkill。**运行中索引更新与 L4 编辑器事件已接线（2026-09-11 第四十四/五十轮）**：L4 见下方面板段；运行中索引更新指「Skill 目录变化后输入区列表无轮询刷新」——`SessionSkillControls.onChange(sessionId, listener)` 转发该 Session 活动作用域的变更订阅，`llm-ui` 的 `bindSkillRefresh` 绑定后拉取一次并在每次通知时重新拉取，`LLMWorkspaceEditor` 订阅/解绑（回归 `packages/llm-ui/src/shell/skill-refresh.test.ts`、`kernel-adapters` 的 `session-skill-controls.test.ts` 与 `skill-device-driver.test.ts`「notifies a Session change subscription…」，以及 `app-shell` 的 `session-skill-panel.test.ts`「refreshes the visible Skill list…」用真实 `ChatInput` 验证「通知 → 重新拉取 → 面板刷新」整链）。该通知只在宿主进程内传播（`SkillDeviceDriver.notifyChange`），跨进程直改 Skill 文件不触发；已在途 Task 的 system 消息不变，新定义在下一次上下文组装（[prompt-context.ts](../../packages/kernel-adapters/src/skill/prompt-context.ts) 的 `refreshScopedSkills`）才生效；load_skill 的正文已通过持久工具输出进入下一次调用。成功加载的 Effect 还快照该 Skill 已注册且 enabled 的工具定义及 external 分类，Agent 下一轮只合并 allowedToolIds 声明范围内的定义，保留原节点同名定义的优先级。Flow 从节点 capabilities 传递此范围，直接聊天从 capabilityPolicy.toolIds 传递并遵守 WebSearch 开关。未提供 allowedToolIds 的旧 Task 不扩展工具列表。动态外部工具沿用 approval=external 的审批；快照不授予执行权限，真正调用仍经过 Session 工具服务与资源授权。
...
Tauri 解析器读取 name/description、trigger-strategy、auto-load、disable-model-invocation、globs、references、template、priority、subagent.role/model 和合法 task-program。name 规范化为小写 ID；正文提取 Compact Instructions 后存入 instructions。autoLoad 由 trigger-strategy 推导（reference 默认 true、action 恒为 false），显式 `auto-load` 只能在 reference 上把它改为 false；这样文件系统来源也能表达「可被 load_skill/面板/`/sk-<id>` 显式加载并登记持久身份，但新运行不自动注入」的定义，用于区分持久身份恢复与 autoLoad。是否静默仍以 disable-model-invocation 为准，单独设置 action 不自动禁用模型调用。
...
Flow 与直接聊天初始化选定 Skill 的关键规则已进入 system 消息。直接聊天经 AgentResolver 获取 capabilityPolicy.skillIds 中的配置定义，排除 disabled/disableModelInvocation/action，将正文和关键规则作为 ContextAssembler 的 skill 来源块写入持久 Task input；空选择不读取 catalog，读取失败在提交 Task 前终止。初始化预算保护规则系统块、允许先移除 skill-index；逐轮裁剪保留已进入 Task 的 system 消息，预算是软上限。**初始化工具激活（直接聊天路径已于 2026-09-11 完成）**：`ConversationRunCoordinator` 用与运行时 `load_skill` 相同的快照形状（`{skillId, compactInstructions, tools}`）为 `capabilityPolicy.skillIds` 中**被选中且可用**的 Skill 生成 `input.skillContexts`（`buildLlmTaskInput` 透传，agent program 在 `initialState` 装载）；工具只取「Skill 声明 ∩ 节点能力白名单 ∩ catalog 中存在定义」的交集并按 catalog 标记 external，因此初始化与独立 `skill.load` 走同一条激活/逐轮关键规则重注入路径，且不扩权。**Flow 节点路径已于同轮接线**：`DurableFlowExecutorOptions.resolveSkillContexts(sessionId, skillIds, allowedToolIds)` 在 `taskSpec` 里按节点 `config.skillIds`（字符串去重，非字符串忽略）取快照并写入 `llm.agent` 输入；委派子节点同样经 `taskSpec`，因此其模板 `skillIds` 也生效。聊天内 Flow 由 `ConversationRunCoordinator` 注入 `skillContextResolver`（复用 `resolveSkills` + `resolveTools`，工具仍取「Skill 声明 ∩ 节点能力 ∩ catalog 定义」交集）；没有端口或没有 Skill 目录的宿主得到空数组，不阻断节点提交。回归：`durable-flow-executor.test.ts`「activates a node's initially selected Skills through the host port」（仅声明 Skill 的节点触发端口、非字符串 id 被忽略、节点输入含快照）与 `direct-skill-context.test.ts` 的 resolver 用例。回归：`packages/llm-session/__tests__/direct-skill-context.test.ts` 断言持久 Task input 的 `skillContexts` 只含被选中的可用 Skill 与其白名单内工具（`llm-tasks` 另有初始化激活/关键规则重注入用例）。app-core 装配的 resolveSessionContext 通过 kernel-adapters 的 buildSkillPromptContext 从同一 Session SkillService 的 getAgentMdContent 获取项目规则；ContextAssembler 保存为 project/system 块，直接聊天和聊天内 Flow 的 Task input 均消费该快照。FlowNodeBinder 在重建节点身份消息时保留 project/session-skill/skill-index 块，systemPromptPolicy=none 显式跳过。规则系统块沿用预算及逐轮裁剪保护，没有另建旧 P0.5 路由器；独立 DAG 命令入口也经同一 resolveSessionContext 获取快照，由执行器在 submit 开始时复制该快照，并在每个 Agent 实例创建时注入，覆盖静态、Composite 展开、graph patch 和委派子节点；没有新用户消息时仅使用空消息匹配及 autoLoad 规则。聊天内 Flow 同样从 ContextSnapshot 提取这三个来源交给执行器；已存在的相同系统消息不重复注入，none 不注入，子节点能力仍取自己的声明。独立入口通过宿主 bindNode 调用 bindStandaloneFlowNode，复用 AgentResolver 和现有身份/模型/Skill/委派模板绑定；不创建聊天历史，节点输入或已有 messages 作为任务正文。静态及 Composite 展开节点、预声明委派模板已解析；独立入口与聊天内 Flow 的运行中 graph patch 节点也调用宿主身份绑定器。执行器先验证原始整批 patch，再异步解析全部新增节点、复验并发布；失败时不提交本批新增节点。动态绑定继承产生节点所属 Flow 的默认身份层：编译产物按节点保存默认配置，Composite 展开后保留子 Flow 作用域，执行器冻结并传给 patch/委派后代；保留新增节点原 capabilities、budget 和委派调度策略，工具目录仍只取原 capabilities，避免身份定义扩权；动态节点内委派模板的 resolvedTemplate 已保留解析后的身份消息与模型，并递归限定各级 capabilities/toolIds 为原模板声明，未声明能力时为空。身份解析不能新增 delegation 或旧 subtasks 委派。Skill 版本解析失败在提交前拒绝，不再降级为空 Skill 列表；Agent 身份缺失仍沿用已有回退语义。关键规则已进入某 Task 后的快照保留，不等于授予相应工具后续执行权限，也不等于完整 Skill 版本冻结。
...
SlashCommandPlugin 为**定义层启用**的 Skill 提供 `/sk-<id>`（按 `definitionEnabled` 过滤，与面板「可否勾选加载」的 `enabled` 区分）。未加载的 action/silent Skill 不能从面板勾选（见下方面板段），`/sk-<id>` 因此是它们唯一的手动词令入口：action/silent 的定义不会被 `load`（模型上下文闸门拒绝），其正文经 `buildActionSkillMessage` 内联进用户消息；可加载的定义则 `load` 后发送 `buildSkillPrompt`。SkillInvocationParser 解析 --key value、Markdown 文件引用、@file、glob 和自由文本；提供 buildActionSkillMessage/buildSkillPrompt 等辅助函数。解析器不是执行器，不能据此声称所有 reference 调用都会走 load_skill 或所有工具类型都已支持。

Skill 上下文组装前，宿主读取 Kernel Session shared 的 kernel-adapters.skills.loaded，并经 sessions.restore 恢复当前来源定义；Effect 路径使用同一恢复入口。**重开语义回归（2026-09-11）**：`session-skill-restore.test.ts` 用真实 runtime + Kernel 验证——首次运行按触发词加载并持久身份后，**新的作用域实例**（等价于重开 Session/换宿主）在用户消息不再匹配时仍按持久身份重新注入正文与关键规则；显式 unload 后同样的新实例不再恢复它，而再次匹配可重新加载（unload 不是永久禁用）。每个活动作用域只恢复一次，并发请求共享恢复过程；销毁/重建阻止旧结果发布。非字符串或空 ID 等损坏记录拒绝，当前禁止模型调用的定义也拒绝；缺失、禁用、越界和支持文件错误继续走 loadSkill 校验。活动作用域内显式 unload 不因每轮读取旧记录而重载，服务层 unloadSkill 仍只修改活动作用域；需要跨重建保留卸载结果时使用 skill.unload Effect。
...
严格版本、自动委派及生命周期故障矩阵的目标行为与验收拆分见 [P2 实施契约](p2-completion.md#2-p2-01-skill-生命周期)。当前 loaded 表支持 format:2（ids/snapshots/drifts），快照包含定义、解析后的正文/支持文件、关键规则及内容摘要。默认 require-reload，可信 Skill 配置可声明 version-policy: keep-old；漂移持久记录旧/新摘要，面板提供显式重新加载。纯 ID 旧记录在执行入口要求 reload，不能伪造旧正文。keep-old 仍复核当前来源、启用状态和工具授权。直接聊天与 Flow 初始 Skill 选择通过 resolveSessionSelectedSkills 使用相同快照。自动委派和完整生命周期故障矩阵仍未完成。
...
**P0-00 严格恢复语义（2026-09-13）**：`auto-load: false` 的 reference Skill 让两个宿主都能用文件系统定义区分三种现象——新运行命中触发词而加载、重开后按 `kernel-adapters.skills.loaded` 持久身份恢复、以及 unload 后不再复活（再次命中仍可加载，unload 非永久禁用）。回归：`kernel-adapters/src/skill/session-file-source.test.ts`（frontmatter 与多层级取舍）、`session-skill-restore.test.ts`（真实 runtime + Kernel，含「从未加载的 Session 不因 autoLoad 获得 Skill」对照）、CLI `apps/cli/tests/run-skill-context.test.ts`（不匹配的新 Run 不自动注入、命中则加载并落盘身份）。
