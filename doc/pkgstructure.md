...
| `@itookit/llm-engine` | Session management + VFS persistence + TaskGraph DAG orchestration + ILoop executors + Plugin system（CommandBus + ExtensionRegistry）+ SessionEventBus + ContextAssembler。所有执行路径统一编译为 TaskGraphRun，通过 TaskGraphReconciler 调度 |
