...
| `@itookit/llm-harness` | Multi-round agent loop — `HarnessLoopExecutor`（AsyncGenerator ILoop, mode='harness'）、harness middleware（Budget/Context/ErrorRecovery/HITL/Skill/BackPressure）、SubAgentRouter |
| `@itookit/llm-runtime` | Session management + VFS persistence + TaskGraph DAG orchestration + ILoop executors + Plugin system（CommandBus + ExtensionRegistry）+ SessionEventBus + ContextAssembler。所有执行路径统一编译为 TaskGraphRun，通过 TaskGraphReconciler 调度 |
