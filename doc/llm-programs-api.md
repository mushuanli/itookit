# @itookit/llm-programs — API 参考

> 平台无关的 LLM Durable Program 层：把"一项 LLM 工作如何向前运行"表达为 Harness `DurableTaskProgram`（init/reduce 状态机）。提供 `llm.chat` / `llm.agent` / `llm.plan` 三个程序、上下文组装与 Provider 消息适配。所有 API 从 `@itookit/llm-programs` 根导出。

**依赖方向**：`llm-programs → harness → common`（+ `llm-common` / `stdio` 类型）。不依赖 `llm-session`、`llm-flow`、UI、DOM 或具体设备；所有外部能力（LLM/Tool）经 Harness Effect 使用。
...
三个 Durable Program 通过 `manifest = { kind, version }` 注册进 Harness，由 `llm-session` 的 `initializeConversationSystem()` 统一注册（`registerPrograms`）：
...
```ts
import { DurableChatProgram, DurableAgentProgram, DurablePlanProgram } from '@itookit/llm-programs';
harness.registerProgram(new DurableChatProgram());
harness.registerProgram(new DurableAgentProgram());
harness.registerProgram(new DurablePlanProgram());
```
...
三个类均实现 Harness `DurableTaskProgram<S, I, O>` 契约（`init` / `reduce` 返回 `Decision`）。
...
`@itookit/llm-programs` 的公共 API 全部从 `packages/llm-programs/src/index.ts` 根导出。包内按 **core / durable** 两层组织：

```
packages/llm-programs/src/
├── index.ts                      根导出（唯一公共入口）
...
**约定**：本包不持有 Session/Flow/Scheduler/CommandBus/通用 Middleware；新运行模式实现 `DurableTaskProgram` 并放入 `durable/`；所有等待必须返回 Harness `WaitSpec`，State 必须可持久化（JSON 可序列化）。
