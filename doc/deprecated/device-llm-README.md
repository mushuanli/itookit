> ⚠️ 已归档：一次性重构方案（把文件迁移到已不存在的 `llm-kernel`/`llm-runtime`，并以 `@itookit/llm-driver` 为包名）；当前文档见 `packages/device-llm/AGENTS.md` 与 `packages/device-llm/doc/driver-details.md`。
