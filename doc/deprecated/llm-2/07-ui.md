...
flowchart LR
    ...
    Events -->|"log:appended"| Reproject["重投影该 roundId 区域<br/>丢弃对应瞬时缓冲"]
    Auth --> P["project()"]
    Trans --> P
    P --> DOM["按 roundId 分区的 DOM<br/><i>区域级替换，非全页重绘</i>"]
    Reproject --> P
```
...
| 流式中 | 瞬时缓冲更新 → 仅重投影当前 round 区域 |
| `log:appended` | 该 round 权威数据到位 → 重投影该区域，覆盖瞬时内容 |
...
沿用项目约定（原生 DOM + 模板字符串 + BEM）。分区键 = `roundId`：`<div data-round-id="01J...">`，重投影 = 该分区 `replaceChildren`。流式 delta 走现有 mdx 增量渲染器。
...
|---|---|
| 普通 round | 现有消息卡片 |
| merge round | 汇合标记 + 策略徽章（concat/summarize/pick）+ 可折叠副支预览 |
| stale round（rebase 未重生成） | 半透明 + "内容基于旧上下文" 角标 + 一键重生成 |
| tag | 书签图标（保存点） |
...
| 问题 | 倾向 |
|---|---|
| 超长会话 DOM 量 | roundId 分区天然支持虚拟滚动（视口外分区不挂载）；首版不做，留接口 |
