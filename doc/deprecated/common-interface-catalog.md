> ⚠️ 已归档：接口清单已失效（`interfaces/fs/`、`IModuleFS`、`ISessionEngine` 等均不存在；VFS 协议已迁至 `@itookit/vfs-core`、LLM 协议已迁至 `@itookit/llm-common`，本包 `src/interfaces/` 仅剩 `ILogger`/`INavigation`/`IHoverPreview`）；当前文档见 `packages/common/AGENTS.md` 与 `doc/interface-contracts.md`。
