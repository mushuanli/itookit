# LLM 四层公共接口审查（kernel / llm-tasks / llm-flow / llm-session）

> 审查目标：接口定义是否**功能清晰、用户友好**（易懂、易用、职责单一、命名一致）。

## 1. 接口清单（当前公共 API）

### 1.1 kernel — 执行内核（30+ 概念）

**顶层入口**：`Kernel`（createSession/openSession/submit/signal/cancel/recover/eventList/…）+ 工具 `bindCapabilities`/`assertEffectGrant`/`interactionApproved`。
...
| 包 | 清晰度 | 说明 |
|---|---|---|
| kernel | ⚠️ 概念多但命名准确 | `submit/signal/chargeBudget/commitContext` 都是精确动词；但 30 方法挤在一个 `SessionHandle` 上，读者很难一眼看出「哪些是一组」 |
| llm-tasks | ✅ 最清晰 | 单一职责（定义 LLM 任务怎么跑），类型扁平、工具函数命名直白 |
...
| 维度 | 现状 | 问题 |
|---|---|---|
| **入口发现** | kernel 要从 `Kernel` 开始；llm-session 要从 `initializeConversationSystem` 开始 | 装配路径长，新人不知道「最小可用」入口 |
...
| **错误模型** | kernel 抛 `Error`（message/code）；llm-session 有 `ConversationError`；llm-flow 有 `FlowDraftVersionConflictError` | 三个包三种错误风格，无统一错误契约 |
...
kernel 定位是内核，30 方法可接受（OS 内核系统调用也上百）。但**没有分层**：把资源/预算/工作区/上下文这些「专家级 API」和 `submit/signal/respond` 这些「日常 API」混在同一个 handle 上。
...
kernel 用裸 `Error` + `code`，llm-session 用 `ConversationError`，llm-flow 用特定 `FlowDraftVersionConflictError`。建议至少给 kernel 定义一个 `KernelError`（带 code 枚举），上层统一捕获。

### 🟢 问题 5（正面）：llm-tasks 与 llm-flow 接口质量高

- llm-tasks 的 `buildLlmTaskInput`/`extractNodeOutput`/`collectDependency` 是优秀的「易用助手」，把繁琐的字段装配/依赖收集/输出提取抽象掉了。
...
| 包 | 功能清晰 | 用户友好 | 主要问题 |
|---|---|---|---|
| kernel | ⚠️ | ⚠️ | `SessionHandle` 30 方法无分层 |
| llm-tasks | ✅ | ✅ | 命名轻微不一致（maxTokens vs total_tokens） |
| llm-flow | ✅ | ✅ | 入口需先拿 sessionId（分层合理） |
| llm-session | ❌ | ❌ | `SessionManager` 50 方法上帝门面 |

**一句话**：llm-tasks 与 llm-flow 的接口是「正面教材」（职责单一 + 易用助手）；kernel 的问题是「宽但没分层」；llm-session 的问题是「上帝门面」。优先级：**先拆 llm-session 的 SessionManager，再给 kernel 的 SessionHandle 分层**。
