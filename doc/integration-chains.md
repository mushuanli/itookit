...
| 步骤 | 组件 | 关键文件 |
|---|---|---|
| 1. 创建 | `createVFS({ rootBackend, modules })` | `stdio/src/impl/factory.ts` |
| 2. 引擎 | `VFSEngine` — 路径解析、系统节点映射 | `stdio/src/impl/engine/` |
| 3. 管理器 | `VFSManager` — module lifecycle | `stdio/src/impl/services/` |
| 4. 模块 | `ModuleFS`（chroot 到 module 目录） | `stdio/src/impl/services/` |
    ...
    → llm-session SessionManager.sendMessage()
      → ConversationRunCoordinator
        → (无工具) session.submit(llm.chat)  |  (有工具) llm.agent
          ...
          → harness drain → DurableAgentProgram.reduce → llm.chat effect
...
| 程序 | `DurableChatProgram` / `DurableAgentProgram` | `llm-programs/src/durable/` |
| 能力绑定 | `bindCapabilities` → capabilities signal | `harness/src/application/capabilities.ts` |
...
cli run (apps/cli)
  ...
  → runtime.ts: createCliRuntime
      ...
      ├─ createCoreutilsRuntime（harness + effect adapters + tools + skills）
      ├─ DurableFlowExecutor（createBuiltinDagPluginRegistry）
      └─ registerPrograms（llm-programs 的 agent/chat/plan + llm-flow 的 flow.*）
  → DurableFlowExecutor.submit(sessionId, DagRunSpec)
      → 就绪节点逐个 session.submit(TaskSpec) → bindCapabilities → start
      → harness drain → 节点 program（llm.agent / flow.value / flow.human …）
...
| 装配 | `createCliRuntime`（harness+coreutils+flow） | `cli/src/runtime.ts` |
...
apps/web-app (entry)
  → initApp() (app-shell)
    → createVFS() → LLMDeviceDriver → new Harness()
      → createCoreutilsRuntime(harness, driver…)（注册 effect/tool/skill）
      → initializeConversationSystem({ agentService, sessionEngine, harness, dagPlugins })
        ├─ registerPrograms（llm-programs + llm-flow 的全部 durable programs）
...
| VFS | `stdio/src/impl/factory.ts::createVFS()` |
| 内核 | `harness/src/application/harness.ts::new Harness()` |
...
DurableAgentProgram.tool 调用（tool.call effect）
  → coreutils ToolCallEffectAdapter（assertEffectGrant → resolveCapability）
    → tools 的 buildTool 实例（FileRead/Bash/…）
      → 经 stdio IDeviceDriver / VFS 访问实际资源
