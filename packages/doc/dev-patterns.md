...
| Session/Task 内核与 Durable 调度 | `harness` |
...
- 新能力通过 Harness 插件注册 `EffectAdapter` 或 Resource provider。
...
```text
common ← llm-runtime → harness
stdio ← harness ← coreutils
common ← llm-conversation → llm-runtime
common ← llm-ui → llm-conversation → harness
app-shell → concrete packages
```

`llm-runtime` 只依赖 common 与 Harness 公共契约，不依赖 Conversation、UI 或平台实现；
UI 不读取 Harness 内部 Store，平台能力实现归属 `apps/*`。
