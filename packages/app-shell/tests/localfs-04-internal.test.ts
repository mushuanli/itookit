...
describe('Internal paths (__config/) — storage location', () => {
    ...
    it('__config/ directory is NOT created in moduleDir', async () => {
        await vfs.fs.createDirectory({ name: '__config', parentIdOrPath: null });
...
describe('Internal paths (__config/) — VFS access', () => {
    ...
    it('can readContent from __config/ path', async () => {
        ...
        const result = await vfs.fs.readContent('/__config/settings.json', { encoding: 'utf-8' });
        expect(result).toBe('{"key":"value"}');
    });

    it('can writeContent to __config/ path', async () => {
        await vfs.fs.createDirectory({ name: '__config', parentIdOrPath: null });
        await vfs.fs.createFile({ name: 'cache.json', parentIdOrPath: '/__config', content: 'v1' });
        await vfs.fs.writeContent('/__config/cache.json', 'v2');
        const result = await vfs.fs.readContent('/__config/cache.json', { encoding: 'utf-8' });
        expect(result).toBe('v2');
    });

    it('can getNode for __config/ directory', async () => {
        await vfs.fs.createDirectory({ name: '__config', parentIdOrPath: null });
        const node = await vfs.fs.getNode('/__config');
...
describe('Internal paths (__config/) — visibility', () => {
    ...
    it('__config/ is hidden from normal getChildren', async () => {
        await vfs.fs.createFile({ name: 'visible.md', parentIdOrPath: null, content: 'hi' });
        await vfs.fs.createDirectory({ name: '__config', parentIdOrPath: null });
        await vfs.fs.createFile({ name: 'data.json', parentIdOrPath: '/__config', content: '{}' });

        const names = (await vfs.fs.getChildren('/')).map(c => c.name);
    ...
    it('moduleDir stays clean — only user files appear there', async () => {
        await vfs.fs.createFile({ name: 'user.md', parentIdOrPath: null, content: 'u' });
        await vfs.fs.createDirectory({ name: '__config', parentIdOrPath: null });
        await vfs.fs.createFile({ name: 'cfg.json', parentIdOrPath: '/__config', content: '{}' });
...
describe('Single _ (assetdir) vs __ (internal) on disk', () => {
    ...
    it('_name/ (assetdir) is real on disk; __name/ is not', async () => {
        await vfs.fs.createFile({ name: 'doc.md', parentIdOrPath: null, content: '# Doc' });
        await vfs.fs.assets!.putAsset('/doc.md', 'img.png', 'image');
        await vfs.fs.createDirectory({ name: '__config', parentIdOrPath: null });
