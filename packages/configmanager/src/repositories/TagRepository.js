...
export class TagRepository {
     ...
     * @param {string[]} [protectedTags=[]] - [新增] 注入受保护的 Tag 列表
     */
    constructor(db, eventManager, protectedTags = []) {
        this.db = db;
        this.events = eventManager;
        this.protectedTags = protectedTags; // [新增] 保存配置
    }
    ...
    async renameTagGlobally(oldTagName, newTagName) {
        // [MODIFIED] 使用注入的配置进行检查
        if (this.protectedTags.includes(oldTagName)) {
        ...
        await new Promise((resolve, reject) => {
            const cursorRequest = nodeTagsIndex.openCursor(IDBKeyRange.only(oldTagName));
            cursorRequest.onsuccess = (event) => {
                // FIX: Cast event.target
                const cursor = (/** @type {IDBRequest<IDBCursorWithValue>} */ (event.target)).result;
                if (cursor) {
    ...
    async deleteTagGlobally(tagName) {
        // [MODIFIED] 使用注入的配置进行检查
        if (this.protectedTags.includes(tagName)) {
        ...
        await new Promise((resolve, reject) => {
            const cursorRequest = nodeTagsIndex.openCursor(IDBKeyRange.only(tagName));
            cursorRequest.onsuccess = (event) => {
                // FIX: Cast event.target
                const cursor = (/** @type {IDBRequest<IDBCursorWithValue>} */ (event.target)).result;
                if (cursor) {
    ...
    async findNodesByTag(tagName) {
        ...
        const nodes = await Promise.all(
            nodeIds.map(id => new Promise(resolve => 
                store.get(id).onsuccess = e => resolve((/** @type {IDBRequest} */ (e.target)).result)
            ))
        );
