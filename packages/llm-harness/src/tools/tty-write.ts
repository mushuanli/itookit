// @file: llm-harness/src/tools/tty-write.ts
...
export function createTtyWriteHandler(
    ...
    return async (args, ctx) => {
        ...
        if (!session) {
            return `Error: TTY session "${sessionId}" not found. Use shell_session to create a new one.`;
        }
        if (session.exited) {
            return `Error: TTY session "${sessionId}" has already exited.`;
        }
