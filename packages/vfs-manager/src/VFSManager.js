/**
 * @file vfsManager/VFSManager.js
 * @fileoverview VFSManager - 虚拟文件系统管理器
 * 主入口类，提供统一的 API
 */
...
export class VFSManager {
    ...
    async init(options = {}) {
        if (this.initialized) {
            console.warn('[VFSManager] Already initialized');
            return;
        }
        
        console.log('[VFSManager] Initializing...');
        ...
        try {
            ...
            console.error('[VFSManager] Initialization failed:', error);
            throw new VFSError(`Failed to initialize VFSManager: ${error.message}`);
        }
    }
    ...
    async shutdown() {
        ...
        console.log('[VFSManager] Shutting down...');
        ...
        console.log('[VFSManager] Shutdown complete');
    }
    ...
    async mount(name, options = {}) {
        ...
        console.log(`[VFSManager] Mounted module: ${name}`);
    ...
    async unmount(name) {
        ...
        console.log(`[VFSManager] Unmounted module: ${name}`);
    ...
    async importModule(data) {
        ...
        for (const nodeData of sortedNodes) {
            try {
                ...
                console.warn(`[VFSManager] Failed to import node ${nodeData.id}:`, error);
            }
        }
        
        console.log(`[VFSManager] Imported module: ${module.name} (${nodes.length} nodes)`);
    }
    ...
    _ensureInitialized() {
        if (!this.initialized) {
            throw new VFSError('VFSManager not initialized. Call init() first.');
        }
    }
