...
import { VFSManager } from '../../src/VFSManager.js';
...
describe('Provider Integration', () => {
    ...
    beforeEach(async () => {
        ...
        await vfsManager.init({ legacyMode: false });
    });
    
    afterEach(async () => {
        await vfsManager.storage.db.disconnect();
    });
...
Embed: ![[diagram-id]]
            ...
            const vnode = await vfsManager.vfs.createNode({
                type: 'file',
                module: 'notes',
                path: '/test.md',
            ...
            // 读取并验证元数据
            const { metadata } = await vfsManager.vfs.read(vnode.id);
    ...
    describe('Provider orchestration', () => {
        it('should maintain derived data consistency on updates', async () => {
            ...
            // 初始状态
            let { metadata } = await vfsManager.vfs.read(vnode.id);
            expect(metadata.clozes).toHaveLength(1);
            
            // 更新：添加一个新挖空
            content = '{{c1::First cloze}} ^clz-1\n{{c1::Second cloze}} ^clz-2';
            await vfsManager.vfs.write(vnode.id, content);
            
            ({ metadata } = await vfsManager.vfs.read(vnode.id));
            expect(metadata.clozes).toHaveLength(2);
...
[[link-target]]
            ...
            // 删除节点
            await vfsManager.vfs.unlink(vnode.id);
            
            // 验证派生数据被清理
            const clozes = await vfsManager.storage.db.getAllByIndex(
                'srsClozes',
                'by_nodeId',
                vnode.id
            );
            ...
            const tasks = await vfsManager.storage.db.getAllByIndex(
                'tasks',
                'by_nodeId',
                vnode.id
            );
            ...
            const links = await vfsManager.storage.db.getAllByIndex(
                'links',
                'by_sourceId',
                vnode.id
            );
    ...
    describe('Provider validation', () => {
        it('should validate SRS content', async () => {
            ...
            const provider = vfsManager.registry.get('srs');
        ...
        it('should validate Task dates', async () => {
            ...
            const provider = vfsManager.registry.get('task');
            const result = await provider.validate(vnode, invalidContent);
