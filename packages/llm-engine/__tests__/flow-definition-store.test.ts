...
import { flowRevisionDigest } from '../src/task-graph/validation';
...
function memoryEngine(): IChatEngine {
    ...
    return {
        ...
        openFile: () => ({ asset: (name: string) => ({ readText: async () => assets.get(name) ?? '' }) }),
...
describe('FlowDefinitionStore', () => {
    it('creates, lists and saves drafts with engine-owned optimistic versions', async () => {
        const store = new FlowDefinitionStore(memoryEngine(), 'harness-v3');
