...
import { flowRevisionDigest, validateFlowRevision } from '../task-graph/validation';
...
export class FlowDefinitionStore {
    constructor(
        ...
        private readonly knownHandlers?: ReadonlySet<string> | (() => ReadonlySet<string>),
    ) {}
    ...
    async saveRevision(revision: FlowRevision): Promise<FlowRevision> {
        const handlers = typeof this.knownHandlers === 'function' ? this.knownHandlers() : this.knownHandlers;
        const issues = validateFlowRevision(revision, handlers);
        if (issues.some(issue => issue.code === 'cycle' || issue.code === 'self-edge' || issue.code === 'unknown-handler')) {
