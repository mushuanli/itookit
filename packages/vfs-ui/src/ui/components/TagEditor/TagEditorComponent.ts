...
export class TagEditorComponent {
  ...
  private handleClick = (e: MouseEvent): void => {
    ...
    const action = (target.closest('.mdx-tag-editor__btn') as HTMLElement)
      ?.dataset.action;
  ...
  private handleInput = async (): Promise<void> => {
    ...
    this.suggestions = query
      ? (await this.params.suggestionProvider.getSuggestions(query)).filter(
          s => !this.items.has(s.label)
        )
      : [];
    this.activeIndex = -1;
  ...
  private removeItem(item: string): void {
    ...
    this.pillsEl
      .querySelector(`[data-item="${escapeHTML(item)}"]`)
      ?.remove();
    this.inputEl.focus();
  }
  ...
  private addPill(item: string): void {
    ...
    this.pillsEl.insertBefore(
      pill,
      this.pillsEl.querySelector('.mdx-tag-editor__input-wrapper')
    );
  }
  ...
  private render(): void {
    ...
    this.suggestionsEl = this.container.querySelector(
      '.mdx-tag-editor__suggestions'
    )!;
  }
