...
export class FileOutline extends BaseComponent<FileOutlineState> {
  ...
  protected bindEvents(): void {
    this.container.addEventListener('click', (e: MouseEvent) => {
      const actionEl = (e.target as Element).closest<HTMLElement>(
        '[data-action]'
      );
      const liEl = (e.target as Element).closest<HTMLElement>(
        'li[data-element-id]'
      );
      ...
      if (action === 'toggle-expand') {
        this.commandBus.execute('ui:toggleOutlineH1', { elementId });
      } else if (action === 'navigate') {
        this.commandBus.execute('nav:navigateToHeading', { elementId });
      }
    });
  }
  ...
  protected render(): void {
    ...
    const createItem = (h: Heading): string => {
      ...
      const children =
        h.level === 1 && hasChildren && isExpanded
