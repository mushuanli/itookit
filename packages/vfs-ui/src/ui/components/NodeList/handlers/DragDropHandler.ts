/**
 * @file vfs-ui/ui/components/NodeList/handlers/DragDropHandler.ts
...
export class DragDropHandler {
  ...
  handleDrop = (event: DragEvent): void => {
    ...
    try {
      ...
      if (
        ...
        payload.sourceInstanceId !== this.instanceId
      ) {
        console.warn('[DragDropHandler] Cross-instance drag ignored');
        this.clearDropIndicators();
        return;
      }
      ...
      if (targetEl && payload.itemIds?.length > 0 && targetEl.dataset.itemId) {
        ...
        if (targetEl.classList.contains('drop-target-above')) position = 'before';
  ...
  private scheduleAutoExpand(target: Element): void {
    ...
    if (!this.getExpandedFolderIds().has(folderId)) {
