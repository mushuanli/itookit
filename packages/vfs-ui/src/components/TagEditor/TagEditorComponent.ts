...
export class TagEditorComponent {
  ...
  constructor(
    ...
    private params: TagEditorParams
  ) {
  ...
  private handleClick = (e: MouseEvent): void => {
    ...
    if (action === 'save') this.params.onSave(this.getItems());
    else if (action === 'cancel') this.params.onCancel();
  };

  private handleInput = async (): Promise<void> => {
    const query = this.inputEl.value;
    this.suggestions = query
      ? (await this.params.suggestionProvider.getSuggestions(query)).filter(s => !this.items.has(s.label))
      : [];
    this.activeIndex = -1;
  ...
  private handleKeyDown = (e: KeyboardEvent): void => {
    ...
    switch (e.key) {
      ...
      case ',':
        e.preventDefault();
        if (n && this.activeIndex > -1) this.addItem(this.suggestions[this.activeIndex].label);
        else if (this.inputEl.value.trim()) this.addItem(this.inputEl.value.trim());
        this.inputEl.value = '';
        this.clearSuggestions();
        break;
      case 'Backspace':
        if (!this.inputEl.value && this.items.size) this.removeItem([...this.items].pop()!);
        break;
      case 'ArrowDown':
        if (n) { e.preventDefault(); this.activeIndex = (this.activeIndex + 1) % n; this.renderSuggestions(); }
        break;
      case 'ArrowUp':
        if (n) { e.preventDefault(); this.activeIndex = (this.activeIndex - 1 + n) % n; this.renderSuggestions(); }
        break;
      case 'Escape':
        e.preventDefault();
        this.suggestions.length ? this.clearSuggestions() : this.params.onCancel();
        break;
    }
  };
  ...
  private addItem(label: string): void {
    ...
    if (!item || this.items.has(item)) { this.inputEl.value = ''; return; }
    this.items.add(item);
    this.addPill(item);
    this.inputEl.focus();
  }
