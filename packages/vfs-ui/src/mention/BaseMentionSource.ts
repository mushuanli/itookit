...
export abstract class BaseMentionSource extends IMentionSource {
    ...
    constructor({ engine, scope = true }: MentionSourceDependencies) {
        super();
        if (!engine) {
            throw new Error(`${this.constructor.name} requires an ISessionEngine instance.`);
        }
        this.engine = engine;
    ...
    protected filterResults(results: EngineNode[]): EngineNode[] {
        return results.filter(node => !shouldFilterNode(node));
    }
    ...
    public abstract getSuggestions(query: string): Promise<Suggestion[]>;
    public abstract getHoverPreview(uri: string): Promise<HoverPreviewData | null>;
}
