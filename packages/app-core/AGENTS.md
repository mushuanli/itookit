...
```
src/
├── index.ts                     统一导出（历史别名 createMindOSRuntime/MindOSRuntime 已移除）
├── runtime/
...
`createApplicationRuntime` 的顺序是：VFS → LLM 驱动 → 核心服务（agent/flow/session files/mounts）→ `createKernelRuntime` → 批量取得 Session 租约后恢复 → 会话系统（`initializeConversationSystem`）→ 默认 Flow 播种 → `RunCatalog`。释放走 `cleanupFns` 逆序 + `AggregateError`。
...
```bash
pnpm --filter @itookit/app-core test        # vitest：测试包含平台无关服务与初始化故障路径
...
平台无关服务测试位于本包 `tests/`，app-shell 保留 UI 与宿主装配测试。基础设施初始化失败时释放已取得的 VFS 与 transport；清理失败与原始异常共同保留。
