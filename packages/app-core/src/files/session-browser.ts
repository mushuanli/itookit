...
class BrowserBackend implements IStorageBackend {
    ...
    async stat(path: string): Promise<FSNode | null> {
        ...
        if (target.kind === 'tasks') return this.node(path, path.endsWith('/@more') ? '更多任务（打开分页列表）' : 'tasks', !path.endsWith('/@more'));
        if (target.kind === 'task') {
    ...
    async list(path: string): Promise<FSNode[]> {
        ...
        if (target.kind === 'tasks') {
            ...
            const nodes = page.items.map(t => this.node(`${path}/${t.id}`, `${t.program.kind} · ${t.status} · ${t.id}`, false, t.updatedAt));
            if (page.nextAfterIndex !== undefined) nodes.push(this.node(`${path}/@more`, '更多任务（打开分页列表）', false));
            return nodes;
        }
...
export function createSessionBrowser(deps: SessionBrowserDependencies) {
    return createFileSystemSource({ tags: false, backend: new BrowserBackend(deps), viewId: 'session-browser:admin', access: 'ro' });
}
