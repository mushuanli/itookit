...
import { reduceOutcome, routeOutcome, transformOutcome } from './operations';
...
export class FlowValueProgram implements DurableTaskProgram<FlowValueState, FlowValueInput, DagNodeOutcome> {
    ...
    reduce(state: Readonly<FlowValueState>, event: TaskInputEvent): Decision<FlowValueState, DagNodeOutcome> {
        ...
        collectDependency(next, event);
        return dependenciesReady(next) ? completeValue(next) : { state: next, next: dependencyWait(next.dependencies) };
    }
}
...
export class FlowHumanProgram implements DurableTaskProgram<FlowHumanState, FlowHumanInput, DagNodeOutcome> {
    ...
    reduce(state: Readonly<FlowHumanState>, event: TaskInputEvent): Decision<FlowHumanState, DagNodeOutcome> {
        ...
        if (next.phase === 'dependencies') {
            collectDependency(next, event);
            if (!dependenciesReady(next)) return { state: next, next: dependencyWait(next.dependencies) };
...
function completeValue(state: FlowValueState): Decision<FlowValueState, DagNodeOutcome> {
    ...
    const operation = state.operation === 'transform'
        ? transformOutcome
        : state.operation === 'reduce' ? reduceOutcome : routeOutcome;
...
function collectDependency(
    ...
    const binding = state.dependencies.find(item => item.taskId === event.taskId);
    if (!binding || state.resolvedDependencyIds.includes(event.taskId)) return;
    const output = selectOutput(event.exit.output, binding.output);
    const current = state.dependencyOutputs[binding.input];
    state.dependencyOutputs[binding.input] = current === undefined ? output : mergeValues(current, output);
    state.resolvedDependencyIds.push(event.taskId);
}

function selectOutput(value: unknown, output = 'result'): JsonValue {
    const record = isRecord(value) ? value : {};
    const outputs = isRecord(record.outputs) ? record.outputs : {};
    if (output in outputs) return jsonValue(outputs[output]);
    if (isRecord(record.message) && 'content' in record.message) return jsonValue(record.message.content);
    return jsonValue(value);
}

function mergeValues(left: JsonValue, right: JsonValue): JsonValue {
    return Array.isArray(left) ? [...left, right] : [left, right];
}

function dependenciesReady(state: {
    dependencies: FlowDependencyBinding[];
    dependencyOutputs: Record<string, JsonValue>;
    resolvedDependencyIds: string[];
}): boolean {
    return state.dependencies.every(item => state.resolvedDependencyIds.includes(item.taskId));
}

function dependencyWait(dependencies: Array<{ taskId: string }>): {
    type: 'wait';
    on: { type: 'all'; waits: Array<{ type: 'task'; id: string }> };
} {
    return {
        type: 'wait',
        on: { type: 'all', waits: dependencies.map(item => ({ type: 'task', id: item.taskId })) },
    };
}
