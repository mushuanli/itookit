...
describe('Symlink operations (IndexedDB backend)', () => {
    ...
    it('symlink returns a node of type symlink', async () => {
        const { fs } = vfs;
        await fs.driver.createFile({ name: 'target.txt', parentIdOrPath: null, content: 'linked' });
    ...
    it('readlink returns the stored target path', async () => {
        const { fs } = vfs;
        await fs.driver.createFile({ name: 'real.txt', parentIdOrPath: null, content: '' });
    ...
    it('readContent through symlink (relative target) reads target content', async () => {
        const { fs } = vfs;
        await fs.driver.createFile({ name: 'data.txt', parentIdOrPath: null, content: 'symlink content' });
    ...
    it('readContent through absolute virtual path symlink reads target', async () => {
        const { fs } = vfs;
        await fs.driver.createFile({ name: 'abs.txt', parentIdOrPath: null, content: 'abs-content' });
    ...
    it('writeContent through symlink updates the target file', async () => {
        const { fs } = vfs;
        await fs.driver.createFile({ name: 'mut.txt', parentIdOrPath: null, content: 'original' });
    ...
    it('relative symlink resolves correctly', async () => {
        const { fs } = vfs;
        await fs.driver.createDirectory({ name: 'sub', parentIdOrPath: null });
        await fs.driver.createFile({ name: 'file.txt', parentIdOrPath: '/sub', content: 'relative-target' });
    ...
    it('symlink to a directory allows getChildren', async () => {
        const { fs } = vfs;
        await fs.driver.createDirectory({ name: 'realdir', parentIdOrPath: null });
        await fs.driver.createFile({ name: 'child.txt', parentIdOrPath: '/realdir', content: '' });
    ...
    it('exists returns true for a valid symlink pointing to an existing file', async () => {
        const { fs } = vfs;
        await fs.driver.createFile({ name: 'ex.txt', parentIdOrPath: null, content: '' });
    ...
    it('readlink on a non-symlink throws EINVAL', async () => {
        const { fs } = vfs;
        await fs.driver.createFile({ name: 'plain.txt', parentIdOrPath: null, content: '' });
