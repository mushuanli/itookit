...
import { Harness } from './application/harness';
import type {
    DurableTaskProgram,
    EffectAdapter,
    EventEnvelope,
...
const binding: StorageBindingRef = { kind: 'test', locator: { rootPath: '/sessions/one/.harness' } };

describe('Harness durable kernel', () => {
    ...
    it('persists and completes a task', async () => {
        harness.registerProgram(echoProgram());
        const session = await harness.createSession({ id: 'session-one', storage: binding });
        ...
        expect(await fs.driver.exists('/sessions/one/.harness/tasks')).toBe(true);
    });

    it('rejects non-durable Task state before committing it', async () => {
        harness.registerProgram(nonDurableStateProgram());
        const session = await harness.createSession({ id: 'session-one', storage: binding });
    ...
    it('prepares resources and signals before a deferred task starts', async () => {
        harness.registerProgram(grantedEffectProgram());
        harness.registerEffect(uppercaseEffect());
        const session = await harness.createSession({ id: 'session-one', storage: binding });
        const task = await session.submit({
    ...
    it('resumes after a durable effect', async () => {
        harness.registerProgram(effectProgram());
        harness.registerEffect(uppercaseEffect());
        const session = await harness.createSession({ id: 'session-one', storage: binding });
        const task = await session.submit<string, string>({
    ...
    it('streams incremental events emitted during an effect to task.events()', async () => {
        harness.registerProgram(streamingEffectProgram());
        harness.registerEffect(streamingEffect());
        const session = await harness.createSession({ id: 'session-one', storage: binding });
        const task = await session.submit<string, string>({
    ...
    it('installs provider capabilities through the public plugin port', async () => {
        const plugin = {
            ...
            install(registration: HarnessRegistration) {
        ...
        await expect(harness.use(plugin)).rejects.toThrow('already installed');
    });

    it('cancels an active effect through its adapter', async () => {
        let cancelCalls = 0;
        harness.registerProgram(recoverableEffectProgram());
        harness.registerEffect(cancellableEffect(() => { cancelCalls++; }));
        const session = await harness.createSession({ id: 'session-one', storage: binding });
        const task = await session.submit({
    ...
    it('ends a hanging effect at its deadline and invokes adapter cancellation', async () => {
        let cancelCalls = 0;
        harness.registerProgram(timeoutEffectProgram());
        harness.registerEffect({
    ...
    it('rejects non-JSON effect requests before persistence', async () => {
        harness.registerProgram(nonJsonEffectProgram());
        const session = await harness.createSession({ id: 'session-one', storage: binding });
        const task = await session.submit({
    ...
    it('renews a long-running effect lease without occupying a task worker', async () => {
        ...
        const first = await configuredHarness(fs, { leaseMs: 30, pollMs: 5 });
        ...
        const second = await configuredHarness(fs);
    ...
    it('recovers and reconciles an effect abandoned by a disposed worker', async () => {
        const first = await configuredHarness(fs, { leaseMs: 5 });
        first.registerProgram(recoverableEffectProgram());
    ...
    it('releases a dependent task after its predecessor succeeds', async () => {
        harness.registerProgram(echoProgram());
        harness.registerProgram(dependencyProgram());
        const session = await harness.createSession({ id: 'session-one', storage: binding });
    ...
    it('durably wakes a task waiter without losing completion', async () => {
        harness.registerProgram(manualProgram());
        harness.registerProgram(taskWaiterProgram());
        const session = await harness.createSession({ id: 'session-one', storage: binding });
    ...
    it('spawns one idempotent child and waits for it atomically', async () => {
        harness.registerProgram(echoProgram());
        harness.registerProgram(spawnParentProgram());
        const session = await harness.createSession({ id: 'session-one', storage: binding });
        ...
        const spawned = (await harness.eventList(session.id, 0))
    ...
    it('supports nested all and quorum durable waits', async () => {
        harness.registerProgram(manualProgram());
        harness.registerProgram(compositeWaitProgram());
        const session = await harness.createSession({ id: 'session-one', storage: binding });
    ...
    it('durably resumes approval and interactive input requests', async () => {
        harness.registerProgram(interactionProgram());
        const session = await harness.createSession({ id: 'session-one', storage: binding });
    ...
    it('commits task state and session shared state atomically', async () => {
        harness.registerProgram(sharedStateProgram());
        const session = await harness.createSession({ id: 'session-one', storage: binding });
        const task = await session.submit({
    ...
    it('persists workspace snapshots, diffs, and merge ancestry through an adapter', async () => {
        ...
        harness.registerWorkspace(workspace.adapter);
        const session = await harness.createSession({ id: 'session-one', storage: binding });
        await session.suspend();
        const owner = await session.submit({ program: { kind: 'test.echo', version: '1' }, input: 'owner' });
        ...
        expect((await harness.eventList(session.id, 0)).map(event => event.type))
    ...
    it('delivers cross-session messages through a durable outbox', async () => {
        ...
        const target = await harness.createSession({
            id: 'session-two',
            storage: { kind: 'test', locator: { rootPath: '/sessions/two/.harness' } },
        });
        expect(await harness.relayPendingMessages()).toBe(1);
        expect((await target.inbox()).map(message => message.payload)).toEqual([{ revision: 1 }]);
        expect(await harness.relayPendingMessages()).toBe(0);
    ...
    it('rebuilds task indexes and catalog routes from task directories', async () => {
        ...
        await fs.meta.seq!.deleteEntry('/sessions/one/.harness/index.seq', `task/${task.id}`);
        await fs.meta.seq!.deleteEntry('/sessions/one/.harness/index.seq', `ready/${task.id}`);
        await fs.meta.seq!.deleteEntry('/.config/harness/catalog.seq', `task/${task.id}`);

        const report = await harness.recover();

        expect(report.rebuiltIndexes).toBeGreaterThanOrEqual(1);
        expect((await (await harness.openTask(task.id)).status()).task.status).toBe('ready');
    });

    it('allows only one worker to claim a task across harness instances', async () => {
        const creator = await configuredHarness(fs, { maxConcurrent: 0 });
        ...
        const first = await configuredHarness(fs);
        const second = await configuredHarness(fs);
    ...
    it('discovers durable ready work without sharing in-memory notifications', async () => {
        const creator = await configuredHarness(fs, { maxConcurrent: 0 });
        const session = await creator.createSession({ id: 'session-one', storage: binding });
        const worker = await configuredHarness(fs, { pollMs: 5 });
    ...
    it('retries a retryable decision after durable backoff', async () => {
        harness.registerProgram(retryOnceProgram());
        const session = await harness.createSession({ id: 'session-one', storage: binding });
        const task = await session.submit({
    ...
    it('fails after exception retry attempts are exhausted', async () => {
        harness.registerProgram(alwaysFailingProgram());
        const session = await harness.createSession({ id: 'session-one', storage: binding });
        const task = await session.submit({
    ...
    it('closes a session and cancels unfinished tasks', async () => {
        ...
        expect((await harness.listSessions().next()).value?.status).toBe('closed');
    });
});
...
async function configuredHarness(
    ...
    const value = new Harness({ catalog: { fs }, ...options });
