// @file: harness/src/application/effect-utils.ts
...
export async function executeEffectWithDeadline(
    ...
    const interrupted = new Promise<never>((_, reject) => {
        ...
        timer = setTimeout(() => {
            const error = harnessError(HarnessErrorCode.EFFECT_TIMEOUT, `Effect timed out after ${effect.timeoutMs}ms`);
...
export function abortError(reason: unknown): Error {
    ...
    const error = harnessError(HarnessErrorCode.EFFECT_CANCELLED, 'Effect execution was cancelled');
