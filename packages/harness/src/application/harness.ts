import { EventBus, type IModuleFS } from '@itookit/stdio';
import { EffectRegistry, ProgramRegistry, StorageResolverRegistry, WorkspaceRegistry } from '../ports/registry';
import type { HarnessPlugin, HarnessRegistration } from '../ports/plugin';
...
export class Harness implements HarnessRegistration {
    ...
    constructor(options: HarnessOptions) {
        ...
        if (this.leaseMs <= 0) throw new Error('Harness leaseMs must be positive');
        if (pollMs < 0) throw new Error('Harness pollMs must be non-negative');
        const catalog = { fs: options.catalog.fs, rootPath: options.catalog.rootPath ?? '/.config/harness' };
        this.store = new SeqFileHarnessStore(catalog, reference => this.resolveStorage(reference));
    ...
    async use(plugin: HarnessPlugin): Promise<void> {
        const key = `${plugin.id}@${plugin.version}`;
        if (this.plugins.has(key)) throw new Error(`Harness plugin already installed: ${key}`);
    ...
    onChanged(listener: (event: HarnessEvents['changed']) => void): () => void {
    ...
    private handlePollError(error: unknown): boolean {
        ...
        console.error('Harness poll failed', error);
        return true;
    }
    ...
    private startLeaseHeartbeat(binding: ResolvedStorageBinding, claim: TaskClaim): () => void {
        const heartbeat = new LeaseHeartbeat({
            ...
            onError: error => console.error('Harness lease heartbeat failed', error),
        });
        const stop = (): void => {
            heartbeat.stop();
    ...
    private startEffectHeartbeat(binding: ResolvedStorageBinding, claim: EffectClaim): () => void {
        const heartbeat = new LeaseHeartbeat({
            ...
            onError: error => console.error('Harness effect heartbeat failed', error),
        });
