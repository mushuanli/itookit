// @file: harness/src/infrastructure/seqfile/store-helpers.ts
// SeqFileHarnessStore 的领域辅助函数（任务、租约、索引、资源、预算、上下文、等待）。
// 底层 SeqFile 原语（路径/键名/编码/事务/事件追加）在 seqfile-core.ts，此处 re-export。
import type { ISeqFileTransaction } from '@itookit/stdio';
import type {
    BudgetAccount,
    ContextBranch,
    ContextCommit,
...
export async function authorizeHandleTx(
    ...
    if (!handle.rights.includes('admin') && !handle.rights.includes(right)) {
        throw harnessError(HarnessErrorCode.HANDLE_LACKS_RIGHT, `Handle ${handle.id} lacks ${right} right`);
    }
    return resource;
}
...
export function assertBudgetCapacity(budget: BudgetAccount, amount: number): void {
    if (budget.used + amount > budget.hardLimit) {
        throw harnessError(HarnessErrorCode.BUDGET_EXCEEDED, `Budget exceeded for ${budget.resourceId}/${budget.dimension}`);
    }
}
