...
import type { Reference } from '@itookit/stdio';
...
describe('Reference operations (IndexedDB backend)', () => {
    ...
    async function collectOutgoing(idOrPath: string, opts?: import('@itookit/stdio').RefQueryOptions): Promise<Reference[]> {
        const refs: Reference[] = [];
        await vfs.fs.meta.refs!.walkOutgoing(idOrPath, (r) => { refs.push(r); return true; }, opts);
        return refs;
    }

    async function collectIncoming(idOrPath: string, opts?: import('@itookit/stdio').RefQueryOptions): Promise<Reference[]> {
        const refs: Reference[] = [];
