...
- `@itookit/common` 反向依赖 stdio(仅 `IEditor` 的 `IModuleFS` 类型 import)。
- `@itookit/vfsdriver-indexeddb` / `@itookit/vfsdriver-localfs` 依赖 stdio,实现 `IStorageBackend`。
- 各业务/UI 包(VFS 类型)直接依赖 stdio。
...
```bash
pnpm --filter @itookit/stdio test   # 引擎 + 协议集成测试
```
