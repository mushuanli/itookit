...
export class SchemaForm {
    ...
    render(): void {
        this.root.innerHTML = renderSchema(asSchema(this.schema), this.value, '$', true);
    }
...
function renderSchema(schema: JsonSchema, value: JsonValue, path: string, root = false): string {
    ...
    return `<label class="task-schema-field"><span>${label}</span>${description}${renderControl(schema, value, path)}</label>`;
}
...
function renderProperties(schema: JsonSchema, value: JsonValue, path: string): string {
    ...
    return Object.entries(schema.properties ?? {}).map(([key, child]) => {
        ...
        return renderSchema({ ...child, title: `${child.title ?? key}${required}` }, (record[key] ?? defaultValue(child)) as JsonValue, `${path}.${key}`);
    }).join('');
}
