import type { FlowDraft, FlowNodeId, FlowNodeDefinition } from '@itookit/common';
...
export class DagCanvas {
    ...
    render(draft: FlowDraft, selectedId?: FlowNodeId): void {
        ...
        this.root.innerHTML = `<div class="dag-canvas__surface" style="transform:scale(${zoom})">
            <svg class="dag-canvas__edges">${draft.edges.map(edge => {
...
function renderNode(
    ...
    return `<article class="dag-node ${selected ? 'is-selected' : ''}"
        ...
        <small>${escapeHTML(node.plugin)}@${escapeHTML(node.pluginVersion)}</small>
