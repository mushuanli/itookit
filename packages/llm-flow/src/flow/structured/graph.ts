...
interface Scope { route: DagNodeDefinition; checks: DagNodeDefinition[]; aggregate: DagNodeDefinition; judge: DagNodeDefinition; }
...
export function compileDispatchGraph<T extends Graph>(graph: T): T {
    ...
    for (const scope of scopes) for (const node of [scope.route, ...scope.checks, scope.aggregate, scope.judge]) {
    ...
    return { ...graph, nodes: nodes.map(node => ({ ...node, config: remapFlowNodeReferences(node.config, aliases), inputs: remapFlowNodeReferences(node.inputs, aliases) as typeof node.inputs })), edges };
}

function isScopeOperator(node: DagNodeDefinition): boolean {
    return ['builtin.check', 'builtin.judge'].includes(node.plugin)
...
function readScope(graph: Graph, route: DagNodeDefinition): Scope {
    ...
    const scope = { route, checks, aggregate, judge };
    validateWiring(graph, scope);
    return scope;
}
function validateWiring(graph: Graph, scope: Scope): void {
    const { route, checks, aggregate, judge } = scope;
    const ids = new Set([route, ...checks, aggregate, judge].map(node => node.id));
    ...
    const feedback = graph.edges.filter(edge => edge.from === judge.id && edge.to === route.id);
    if (feedback.length !== 1 || feedback[0].kind !== 'control' || feedback[0].output !== 'repeat') throw new Error('Judge requires one repeat control edge to its route');
    ...
    for (const edge of graph.edges) {
        ...
        if (edge.from === judge.id && edge.to !== route.id && edge.output && edge.output !== 'result') throw new Error('Judge output must use result or repeat');
    }
}
function validateOperator(node: DagNodeDefinition): void {
    if (Object.keys(node.retry ?? {}).length || Object.keys(node.budget ?? {}).length || node.compensate || (node.priority !== undefined && node.priority !== 0) || node.capabilities?.length
...
function compileScope(scope: Scope): DagNodeDefinition {
    ...
    const config = { ...route, branches, until, maxRounds: judge.maxRounds,
        ...
        ...(judge.revision ? { revision: judge.revision } : {}),
...
function compileCheck(node: DagNodeDefinition, judge: Record<string, unknown>): DispatchBranch {
    const { key, bindings, when, prompt, outputContract, output, outputFormat, outputSchema, validate, context, publishToHistory, ...agent } = object(node.config);
    if (typeof agent.systemPrompt === 'string') agent.systemPrompt = [agent.systemPrompt];
    const branchKey = typeof key === 'string' ? key : node.id;
    return { key: branchKey, prompt: prompt as string | undefined, outputContract: outputContract as DispatchBranch['outputContract'], input: (bindings ?? {}) as DispatchBranch['input'],
        target: { ...node, plugin: 'builtin.agent', config: agent } as DispatchBranch['target'],
        when: (when ?? ((judge.until || judge.condition) ? undefined : { kind: 'not', args: [criterion(branchKey, judge)] })) as SerializableExpression | undefined,
