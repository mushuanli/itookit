      ...
      renderer: {
        ...
        listitem(token: any): string {
          ...
          if (isTask) {
            ...
            const checkbox = `<input type="checkbox" class="mdx-task-item" ${isChecked ? 'checked' : ''
        ...
        tablecell(content: string, flags): string {
          ...
          const processedContent = safeContent.replace(/\[([ xX])\]/gi, (_match, state) => {
            ...
            const index = self.renderTaskCounter++;
            return `<input type="checkbox" class="mdx-task-item mdx-table-task" ${isChecked ? 'checked' : ''
              } data-task-index="${index}">`;
          });
