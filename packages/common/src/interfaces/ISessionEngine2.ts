 ...
 * 触发时机: copy
 */
export interface NodeCopiedPayload {
  /** 源节点 ID */
  sourceId: string;
  /** 新创建的目标节点 ID */
  targetId: string;
 ...
 */
export interface BatchMovedPayload {
  /** 所有被移动节点的详情 */
  movedNodes: Array<{
    nodeId: string;
    oldPath: string;
 ...
 * 触发时机: copyNodes
 */
export interface BatchCopiedPayload {
  /** 所有复制操作的详情 */
  copies: Array<{
    sourceId: string;
    targetId: string;
...
export interface EngineEventPayloadMap {
  ...
  'node:deleted': NodeDeletedPayload;
  'node:moved': NodeMovedPayload;
  'node:copied': NodeCopiedPayload;
  'node:renamed': NodeRenamedPayload;
  'node:batch_created': BatchCreatedPayload;
  'node:batch_updated': BatchUpdatedPayload;
  'node:batch_moved': BatchMovedPayload;
  'node:batch_deleted': BatchDeletedPayload;
  'node:batch_copied': BatchCopiedPayload;
  'error': ErrorPayload;
}
...
export interface EngineEvent<T extends EngineEventType = EngineEventType> {
  type: T;
  /** 事件载荷，类型由 EngineEventPayloadMap 推导 */
  payload: T extends keyof EngineEventPayloadMap ? EngineEventPayloadMap[T] : unknown;
  /** 事件发生的时间戳 (ms) */
  timestamp: number;
}
 ...
 * - 可选方法(带 ?) 允许后端增量实现，Service 层对缺失方法提供 fallback
 */
export interface ISessionEngine {
   ...
   * @param idOrPath - 节点 ID 或模块内路径
   */
  getNode(idOrPath: string): Promise<EngineNode | null>;

  /**
   * 根据路径获取节点
   * 路径为模块内的相对路径（如 `/notes/hello.md`）
   * 不存在时返回 null
   */
  getNodeByPath?(path: string): Promise<EngineNode | null>;
   ...
   *
   * @emits node:created → NodeCreatedPayload
   */
  createFile(
    name: string,
    parentId: string | null,
   ...
   *
   * @emits node:batch_created → BatchCreatedPayload
   */
  createFiles?(
    files: Array<{ title: string; content: string | ArrayBuffer }>,
    parentId: string | null
  ): Promise<EngineNode[]>;
   ...
   * 写入/覆盖文件内容
   *
   * @emits node:updated → NodeUpdatedPayload { changedFields: ['content'] }
   */
   ...
   *
   * @emits node:renamed → NodeRenamedPayload
   * @emits node:moved  → NodeMovedPayload  (底层 VFS 仍发出 moved)
   */
   ...
   *
   * @emits node:moved       → NodeMovedPayload       (单节点)
   * @emits node:batch_moved → BatchMovedPayload      (多节点)
   */
   ...
   *
   * @emits node:deleted       → NodeDeletedPayload       (单节点)
   * @emits node:batch_deleted → BatchDeletedPayload      (多节点)
   */
   ...
   *
   * @emits node:updated → NodeUpdatedPayload { changedFields: ['metadata'] }
   */
   ...
   * @returns 新创建的节点
   *
   * @emits node:copied → NodeCopiedPayload
   */
  copy?(
    sourceId: string,
    targetParentId: string | null,
   ...
   *
   * @emits node:batch_copied → BatchCopiedPayload
   */
  copyNodes?(
    sourceIds: string[],
    targetParentId: string | null
   ...
   * 设置节点的标签 (全量替换模式)
   *
   * @emits node:updated → NodeUpdatedPayload { changedFields: ['tags'] }
   */
   ...
   *
   * @emits node:batch_updated → BatchUpdatedPayload { reason: 'tags' }
   */
   ...
   *
   * @emits node:batch_updated → BatchUpdatedPayload { reason: 'metadata' }
   */
  updateMetadataBatch?(
   ...
   * 订阅数据变更事件
   *
   * 类型安全：回调中的 event.payload 类型由 EngineEventPayloadMap[E] 自动推导。
   *
   * @example
   * ```ts
   * const unsub = engine.on('node:created', (event) => {
   *   // event.payload 自动推导为 NodeCreatedPayload
  ...
  on<E extends EngineEventType>(
    event: E,
    callback: (event: EngineEvent<E>) => void
  ): () => void;
}
