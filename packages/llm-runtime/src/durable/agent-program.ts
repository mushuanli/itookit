...
function collect(state: DurableAgentState, event: TaskInputEvent): Decision<DurableAgentState, DurableAgentOutput> {
    const dependency = dependencyOutput(event, state.input.dependencyBindings);
    if (dependency && !state.resolvedDependencyIds.includes(dependency.taskId)) {
        mergeDependencyOutput(state.dependencyOutputs, dependency.key, dependency.value);
        state.resolvedDependencyIds.push(dependency.taskId);
    }
    const capabilities = capabilitySignal(event);
    if (capabilities) state.capabilities = capabilities;
    if (!state.capabilities || !dependenciesReady(state)) return { state, next: waitForInput(state) };
...
function handleInteraction(
    ...
    if (state.phase === 'approval' && !approved(event.value)) {
...
function dependenciesReady(state: DurableAgentState): boolean {
...
function approved(value: JsonValue): boolean {
