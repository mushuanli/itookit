import type { ChatMessage, ContextCompactionPolicy } from '@itookit/common';

export function validateContextCompaction(policy?: ContextCompactionPolicy): void {
    if (!policy) return;
    for (const value of [policy.maxMessages, policy.keepRecent ?? 1]) {
...
export function compactMessages(messages: ChatMessage[], policy?: ContextCompactionPolicy): ChatMessage[] {
    ...
    messages.forEach((message, index) => {
        if (message.role === 'system' || index >= messages.length - recent) keep.add(index);
