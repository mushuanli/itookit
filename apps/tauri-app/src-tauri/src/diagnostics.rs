...
use std::{io::Write, path::{Path, PathBuf}, sync::{Mutex, OnceLock}};

static LOG: OnceLock<DiagnosticLog> = OnceLock::new();
struct DiagnosticLog { path: PathBuf, marker: PathBuf, lock: Mutex<()> }

fn stamp() -> u128 {
    std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis()
}

fn append(path: &Path, event: &str, detail: Value) -> std::io::Result<()> {
...
pub fn record(event: &str, detail: Value) {
    if let Some(log) = LOG.get() {
        if let Ok(_guard) = log.lock.try_lock() {
            if let Err(error) = append(&log.path, event, detail) { eprintln!("[Diagnostics] {error}"); }
        } else { let _ = append(&log.path.with_extension("emergency.jsonl"), event, detail); }
    }
}

pub fn install() {
    let base = std::env::var_os("MINDOS_DIAGNOSTICS_DIR").map(PathBuf::from).unwrap_or_else(|| {
        std::env::var_os("XDG_CONFIG_HOME").map(PathBuf::from)
            .unwrap_or_else(|| PathBuf::from(std::env::var_os("HOME").unwrap_or_default()).join(".config"))
            .join("mindos/logs/desktop")
    });
    if let Err(error) = std::fs::create_dir_all(&base) { eprintln!("[Diagnostics] Cannot create {}: {error}", base.display()); return; }
    ...
    let _ = LOG.set(DiagnosticLog { path, marker, lock: Mutex::new(()) });
    record("process.start", json!({"cwd": std::env::current_dir().ok(), "version": env!("CARGO_PKG_VERSION")}));
    install_panic_hook();
    spawn_watcher();
}
...
fn install_panic_hook() {
    ...
    std::panic::set_hook(Box::new(move |info| {
        record("rust.panic", json!({"message": info.to_string(), "backtrace": std::backtrace::Backtrace::force_capture().to_string()}));
        previous(info);
    }));
}

pub fn clean_exit() {
    record("process.exit", json!({"clean": true}));
    if let Some(log) = LOG.get() { let _ = std::fs::remove_file(&log.marker); }
}
...
fn recover_markers(base: &Path) {
    ...
    for entry in entries.flatten() {
        let path = entry.path();
        if path.extension().and_then(|ext| ext.to_str()) != Some("active") { continue; }
        let pid = std::fs::read_to_string(&path).ok().and_then(|v| v.parse::<u32>().ok());
        #[cfg(target_os = "linux")]
        if pid.is_some_and(process_alive) { continue; }
        #[cfg(not(target_os = "linux"))]
        { let _ = pid; continue; }
        let _ = append(&path.with_extension("jsonl"), "process.previous_unclean_exit", json!({"pid": pid, "reason": "unknown; inspect OS OOM/coredump logs"}));
...
fn spawn_watcher() {
    ...
    if let (Some(log), Ok(exe)) = (LOG.get(), std::env::current_exe()) {
        ...
        let result = Command::new(exe).arg("--mindos-diagnostic-watch").arg(std::process::id().to_string())
            .arg(&log.marker).arg(&log.path).stdin(Stdio::null()).stdout(Stdio::null()).stderr(Stdio::null()).spawn();
        if let Err(error) = result { record("watcher.failed", json!({"error": error.to_string()})); }
    }
}
...
pub fn run_watcher() -> bool {
    ...
    if args.get(1).map(String::as_str) != Some("--mindos-diagnostic-watch") { return false; }
    if args.len() != 5 || args[2].parse::<u32>().is_err() { return true; }
    let marker = Path::new(&args[3]);
    while marker.exists() && process_alive(args[2].parse().unwrap()) {
        std::thread::sleep(std::time::Duration::from_secs(1));
    }
    if marker.exists() {
        let _ = append(Path::new(&args[4]), "process.unexpected_exit", json!({"pid": args[2], "reason": "unknown; no clean exit observed"}));
...
fn process_alive(pid: u32) -> bool {
    std::fs::read_to_string(format!("/proc/{pid}/stat")).ok()
        .and_then(|text| text.rsplit_once(") ").map(|(_, fields)| !fields.starts_with('Z'))).unwrap_or(false)
}
...
mod tests {
    ...
    fn appends_parseable_diagnostics_and_rotates_large_logs() {
        ...
        append(&path, "rust.panic", json!({"message": "line one\nline two"})).unwrap();
        let row: Value = serde_json::from_str(&std::fs::read_to_string(&path).unwrap()).unwrap();
        assert_eq!(row["event"], "rust.panic");
        std::fs::OpenOptions::new().write(true).open(&path).unwrap().set_len(4 * 1024 * 1024 + 1).unwrap();
        append(&path, "process.exit", json!({})).unwrap();
