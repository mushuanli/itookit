// @file: app/workspace/settings/editors/MCPSettingsEditor.ts
import { BaseSettingsEditor } from './BaseSettingsEditor';
import { MCPServer } from '../types';
import { generateShortUUID } from '@itookit/common'; // 确保路径正确，或实现一个简单的uuid函数
import { Toast, Modal } from '../components/UIComponents';

export class MCPSettingsEditor extends BaseSettingsEditor {
    private selectedId: string | null = null;

    render() {
        const servers = this.service.getMCPServers();
    ...
    private async saveCurrentServer() {
        ...
        const existing = this.service.getMCPServers().find(s => s.id === this.selectedId);
        if (!existing) return;
    ...
    private async testConnection() {
        ...
        try {
            ...
            const server = this.service.getMCPServers().find(s => s.id === this.selectedId);
            if (server) {
    ...
    private deleteTool(index: number) {
        const server = this.service.getMCPServers().find(s => s.id === this.selectedId);
    ...
    private exportAll() {
        const data = JSON.stringify(this.service.getMCPServers(), null, 2);
