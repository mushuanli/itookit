审查分析梳理 @packages/vfs-core, 这是一个基础库，为应用提供基本的文件操作， 包括 asset dir 架构使得一个文件可以带有其他多媒体信息，类似压缩包方式； 以及 seq file 为上层提供简单数据库屏蔽。接口定义是否正确合理？事件流是否合适？内部流程是否清晰简明？内部模块划分是否正确解耦容易扩展？代码质量是否可以进一步精简提高？
...
审查分析梳理 @packages/common/src/interfaces/fs/ 和 @packages/vfs-core/ 逻辑，提出重构方案。当前vfs存在相对普通文件系统有如下扩展：1. 一个文件可以拥有assetdir,vfs文件可以包含其他文件组成复杂内容; 2.
